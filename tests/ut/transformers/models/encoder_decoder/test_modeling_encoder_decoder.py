...
# ============================================================================
""" Testing suite for the Mindspore EncoderDecoder model. """
...
from mindnlp.utils.testing_utils import CaptureLogger, require_mindspore, slow
...
class EncoderDecoderMixin:
    ...
    def create_and_check_encoder_decoder_shared_weights(
        ...
        **kwargs,
        ...
        if model.decoder.base_model_prefix == 'bert':
            ms.save_checkpoint(model.decoder.bert, 'bert.ckpt')
            decoder_state_dict = ms.load_checkpoint('bert.ckpt')
        if model.decoder.base_model_prefix == 'roberta':
            ms.save_checkpoint(model.decoder.roberta, 'roberta.ckpt')
            decoder_state_dict = ms.load_checkpoint('roberta.ckpt')

        ms.load_param_into_net(model.encoder, decoder_state_dict)
        ...
        self.assertLess(sum(p.numel() for _, p in tied_model.parameters_and_names()),
                        sum(p.numel() for _, p in model.parameters_and_names()))
        random_slice_idx = ids_tensor((1,), model_result[0].shape[-1]).item()

        # print('==============model_result===============')
        # print(model_result[0][0, :, random_slice_idx].asnumpy())
        # print('==============tied_model_result===============')
...
class BertEncoderDecoderModelTest(EncoderDecoderMixin, unittest.TestCase):
    ...
    def test_bert2bert_summarization(self):
        ...
        output_ids = model.generate(
            input_dict["input_ids"], attention_mask=input_dict["attention_mask"]
        )
    ...
    def test_bert2bert_default_decoder_attention_mask(self):
        ...
        encoder_model, decoder_model = self.get_encoder_decoder_model(
            encoder_config, decoder_config)
        model = EncoderDecoderModel(
            config=config, encoder=encoder_model, decoder=decoder_model)
        ...
        attention_mask = ms.Tensor(input_ids != 5, input_ids.dtype)
        labels = ms.Tensor(
            [
        ...
        self.assertNotIn(
            "We strongly recommend passing in an `attention_mask`", cl.out)
        ...
        attention_mask_ignoring_padding = ops.ones(
            labels.shape, dtype=ms.int64)
        ms.set_seed(0)
...
class BertGenerationEncoderDecoderModelTest(EncoderDecoderMixin, unittest.TestCase):
    ...
    def test_roberta2roberta_summarization(self):
        ...
        input_dict = tokenizer(
            [ARTICLE_PS3, ARTICLE_TOSHIBA], max_length=512, padding="max_length", return_tensors="ms"
        )
        output_ids = model.generate(
            input_dict["input_ids"], attention_mask=input_dict["attention_mask"]
        )
...
class GPT2EncoderDecoderModelTest(EncoderDecoderMixin, unittest.TestCase):
    ...
    def test_bert2gpt2_summarization(self):
        ...
        tokenizer_in = AutoTokenizer.from_pretrained(
        ...
        input_dict = tokenizer_in(ARTICLE_STUDENTS, return_tensors="ms")
        output_ids = model.generate(input_dict["input_ids"])
