...
# ============================================
""" Testing suite for the Mindspore PVT2 Model. """
...
if is_mindspore_available():
    ...
    from mindnlp.transformers import AutoImageProcessor, PvtV2ForImageClassification, PvtV2Model, PvtV2Backbone, PvtV2Config
...
class PvtV2ModelTester(ModelTesterMixin):
    ...
    def create_and_check_backbone(self, config, pixel_values, labels):
        ...
        self.parent.assertListEqual(list(result.feature_maps[0].shape), [
...
@require_mindspore
class PvtV2ModelTest(ModelTesterMixin, unittest.TestCase):
    all_model_classes = (PvtV2ForImageClassification,)
            ...
            self.assertListEqual(
                ...
                [
                    ...
                    self.model_tester.image_size // 2 ** (
                        2 + self.model_tester.out_indices[0]),
                    self.model_tester.image_size // 2 ** (
                        2 + self.model_tester.out_indices[0]),
                ],
            )
...
class PvtV2ModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_image_classification(self):
        ...
        expected_slice = ms.Tensor([-1.4192, -1.9158, -0.9702])
    ...
    def test_inference_model(self):
        ...
        # verify the logits
        expected_shape = (1, 50, 512)
        self.assertEqual(outputs.last_hidden_state.shape, expected_shape)

        expected_slice = ms.Tensor(
            [[-0.3086, 1.0402, 1.1816], [-0.2880, 0.5781, 0.6124],
                [0.1480, 0.6129, -0.0590]]
        )

        self.assertTrue(np.allclose(outputs.last_hidden_state[0, :3, :3].asnumpy(
        ), expected_slice.asnumpy(), atol=1e-4))
    ...
    def test_inference_fp16(self):
        ...
        """
        model = PvtV2ForImageClassification.from_pretrained(
            "OpenGVLab/pvt_v2_b0", ms_dtype=ms.float16)
        image_processor = AutoImageProcessor.from_pretrained(
            "OpenGVLab/pvt_v2_b0")

        image = prepare_img()
        inputs = image_processor(images=image, return_tensors="ms")
        pixel_values = inputs.pixel_values.astype(ms.float16)
...
class PvtV2BackboneTest(BackboneTesterMixin, unittest.TestCase):
    all_model_classes = (PvtV2Backbone,)
    has_attentions = False
    ...
    def test_config(self):
        ...
        num_stages = len(config.depths) if hasattr(
            config, "depths") else config.num_hidden_layers
        expected_stage_names = [
            f"stage{idx}" for idx in range(1, num_stages + 1)]
        self.assertEqual(config.stage_names, expected_stage_names)
        self.assertTrue(set(config.out_features).issubset(
            set(config.stage_names)))
        ...
        config = config_class(
            out_features=["stage1", "stage2"], out_indices=[0, 1])
        ...
        self.assertEqual(config.out_features, [
                         config.stage_names[0], config.stage_names[2]])
        self.assertEqual(config.out_indices, [0, 2])

        # Error raised when out_indices do not correspond to out_features
        with self.assertRaises(ValueError):
            config = config_class(
                out_features=["stage1", "stage2"], out_indices=[0, 2])
