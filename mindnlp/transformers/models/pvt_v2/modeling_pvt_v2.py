...
class PvtV2OverlapPatchEmbeddings(nn.Cell):
    ...
    def __init__(self, config: PvtV2Config, layer_idx: int):
        ...
        num_channels = config.num_channels if layer_idx == 0 else config.hidden_sizes[
            layer_idx - 1]
        ...
        self.padding = (patch_size[1] // 2, patch_size[1] //
                        2, patch_size[0] // 2, patch_size[0] // 2)
        self.layer_norm = nn.LayerNorm(
            hidden_size, epsilon=config.layer_norm_eps)

    def construct(self, pixel_values):
        pixel_values = ops.pad(pixel_values, self.padding)
...
class PvtV2DepthWiseConv(nn.Cell):
    ...
    def __init__(self, config: PvtV2Config, dim: int = 768):
        ...
        self.dwconv = nn.Conv2d(
            dim, dim, 3, 1, pad_mode='valid', group=dim, has_bias=True)

    def construct(self, hidden_states, height, width):
        batch_size, seq_len, num_channels = hidden_states.shape
        hidden_states = hidden_states.swapaxes(1, 2).view(
            batch_size, num_channels, height, width)
        hidden_states = ops.pad(hidden_states, [1, 1, 1, 1])
...
class PvtV2SelfAttention(nn.Cell):
    ...
    def __init__(self, config: PvtV2Config, hidden_size: int, num_attention_heads: int, spatial_reduction_ratio: int):
        ...
        self.proj = nn.Dense(self.hidden_size, self.hidden_size)
        ...
        if self.linear_attention:
            ...
            self.spatial_reduction = nn.Conv2d(
                self.hidden_size, self.hidden_size, kernel_size=1, stride=1, has_bias=True)
            self.layer_norm = nn.LayerNorm(
                self.hidden_size, epsilon=config.layer_norm_eps)
            self.act = nn.GELU()
        elif spatial_reduction_ratio > 1:
            self.spatial_reduction = nn.Conv2d(
                self.hidden_size, self.hidden_size, kernel_size=spatial_reduction_ratio, stride=spatial_reduction_ratio, has_bias=True
            )
            self.layer_norm = nn.LayerNorm(
                self.hidden_size, epsilon=config.layer_norm_eps)
    ...
    def construct(
        ...
        if self.linear_attention:
            hidden_states = hidden_states.permute(0, 2, 1).reshape(
                batch_size, num_channels, height, width)
            hidden_states = (
                self.spatial_reduction(self.pool(hidden_states)).reshape(
            ...
            hidden_states = hidden_states.permute(0, 2, 1).reshape(
                batch_size, num_channels, height, width)
            hidden_states = (
                self.spatial_reduction(hidden_states).reshape(
                    batch_size, num_channels, -1).permute(0, 2, 1)
            )
        ...
        context_layer = (attention_probs @ value_layer).swapaxes(1,
                                                                 2).reshape(batch_size, seq_len, num_channels)
    ...
    def prune_heads(self, heads):
        ...
        self.proj = prune_linear_layer(self.proj, index, axis=1)
...
class PvtV2EncoderLayer(nn.Cell):
    def __init__(self, config: PvtV2Config, layer_idx: int):
        ...
        for block_idx in range(config.depths[layer_idx]):
            block_layers.append(
                PvtV2BlockLayer(
                    config=config,
                    layer_idx=layer_idx,
                    drop_path=drop_path_decays[sum(
                        config.depths[:layer_idx]) + block_idx],
                )
            )
        ...
        # Layer norm
        self.layer_norm = nn.LayerNorm(
            config.hidden_sizes[layer_idx], epsilon=config.layer_norm_eps)
    ...
    def construct(self, hidden_states, output_attentions):
        ...
        for block in self.blocks:
            layer_outputs = block(hidden_states, height,
                                  width, output_attentions)
...
class PvtV2Encoder(nn.Cell):
    def __init__(self, config: PvtV2Config):
        ...
        # encoder layers
        self.layers = nn.CellList([PvtV2EncoderLayer(config, i)
...
class PvtV2PreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, cell: Union[nn.Dense, nn.Conv2d, nn.LayerNorm]) -> None:
        ...
        if isinstance(cell, nn.Dense):
            ...
            fan_out = cell.kernel_size[0] * \
                cell.kernel_size[1] * cell.out_channels
            fan_out //= cell.group
            cell.weight.data.initialize(Normal(math.sqrt(2.0 / fan_out)))
            if cell.bias is not None:
                cell.bias.initialize('zeros')
