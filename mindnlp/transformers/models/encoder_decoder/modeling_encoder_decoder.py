...
# ============================================================================
""" MindSpore EncoderDecoder model. """
...
from mindnlp.transformers.modeling_outputs import BaseModelOutput, Seq2SeqLMOutput
...
def shift_tokens_right(input_ids: ms.Tensor, pad_token_id: int, decoder_start_token_id: int):
    ...
    shifted_input_ids[:, 1:] = ms.Tensor(input_ids[:, :-1])
...
class EncoderDecoderModel(PreTrainedModel):
    ...
    def __init__(
        ...
        decoder_signature = set(inspect.signature(
            self.decoder.construct).parameters.keys())
    ...
    def tie_weights(self):
        ...
        if self.config.tie_encoder_decoder:
            ...
            for m in self.decoder.cells_and_names():
                name = m[0]
                if name == decoder_base_model_prefix:
                    decoder_cell = m[1]
    ...
    def construct(
        ...
        past_key_values: Tuple[Tuple[ms.Tensor]] = None,
        ...
        **kwargs,
        ...
        if encoder_outputs is None:
            encoder_outputs = self.encoder(
                ...
                attention_mask=attention_mask.astype(
                    ms.float32) if attention_mask is not None else attention_mask,
        ...
        if (labels is not None) and (decoder_input_ids is None and decoder_inputs_embeds is None):
            ...
            if decoder_attention_mask is None:
                decoder_attention_mask = ms.Tensor(
                    decoder_input_ids != self.config.pad_token_id, dtype=decoder_input_ids.dtype)

        # Decode
        decoder_outputs = self.decoder(
            input_ids=decoder_input_ids,
            attention_mask=decoder_attention_mask.astype(
                ms.float32) if decoder_attention_mask is not None else decoder_attention_mask,
            encoder_hidden_states=encoder_hidden_states,
            encoder_attention_mask=attention_mask.astype(
                ms.float32) if attention_mask is not None else attention_mask,
