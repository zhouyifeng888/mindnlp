...
class PvtPatchEmbeddings(nn.Cell):
    """
    ...
    def __init__(
        ...
        self.position_embeddings = ms.Parameter(
            ops.randn(1, num_patches +
                      1 if cls_token else num_patches, hidden_size)
        )
        self.cls_token = ms.Parameter(
            ops.zeros(1, 1, hidden_size)) if cls_token else None
        self.projection = nn.Conv2d(
            num_channels, hidden_size, kernel_size=stride, stride=patch_size, has_bias=True)
    ...
    def interpolate_pos_encoding(self, embeddings: ms.Tensor, height: int, width: int) -> ms.Tensor:
        ...
        embeddings = embeddings.reshape(
            1, height, width, -1).permute(0, 3, 1, 2)
        interpolated_embeddings = ops.interpolate(
            embeddings, size=(height, width), mode="bilinear")
        interpolated_embeddings = interpolated_embeddings.reshape(
            1, -1, height * width).permute(0, 2, 1)
        return interpolated_embeddings

    def construct(self, pixel_values: ms.Tensor) -> Tuple[ms.Tensor, int, int]:
        ...
        *_, height, width = patch_embed.shape
        patch_embed = patch_embed.flatten(start_dim=2).swapaxes(1, 2)
        embeddings = self.layer_norm(patch_embed)
        if self.cls_token is not None:
            cls_token = self.cls_token.expand(batch_size, -1, -1)
            embeddings = ops.cat((cls_token, embeddings), axis=1)
            position_embeddings = self.interpolate_pos_encoding(
                self.position_embeddings[:, 1:], height, width)
            position_embeddings = ops.cat(
                (self.position_embeddings[:, :1], position_embeddings), axis=1)
        else:
            position_embeddings = self.interpolate_pos_encoding(
...
class PvtEfficientSelfAttention(nn.Cell):
    ...
    def __init__(
        ...
        self.query = nn.Dense(
            self.hidden_size, self.all_head_size, has_bias=config.qkv_bias)
        self.key = nn.Dense(
            self.hidden_size, self.all_head_size, has_bias=config.qkv_bias)
        self.value = nn.Dense(
            self.hidden_size, self.all_head_size, has_bias=config.qkv_bias)
        ...
        if sequences_reduction_ratio > 1:
            self.sequence_reduction = nn.Conv2d(
                hidden_size, hidden_size, kernel_size=sequences_reduction_ratio, stride=sequences_reduction_ratio, has_bias=True
            )
            self.layer_norm = nn.LayerNorm(
                hidden_size, epsilon=config.layer_norm_eps)

    def transpose_for_scores(self, hidden_states: int) -> ms.Tensor:
        new_shape = hidden_states.shape[:-1] + \
    ...
    def construct(
        ...
        if self.sequences_reduction_ratio > 1:
            ...
            hidden_states = hidden_states.reshape(
                batch_size, num_channels, -1).permute(0, 2, 1)
...
class PvtAttention(nn.Cell):
    ...
    def construct(
        self, hidden_states: ms.Tensor, height: int, width: int, output_attentions: bool = False
    ) -> Tuple[ms.Tensor]:
...
class PvtLayer(nn.Cell):
    def __init__(
        ...
        self.drop_path = PvtDropPath(
            drop_path) if drop_path > 0.0 else nn.Identity()
        self.layer_norm_2 = nn.LayerNorm(
            hidden_size, epsilon=config.layer_norm_eps)
        mlp_hidden_size = int(hidden_size * mlp_ratio)
        self.mlp = PvtFFN(config=config, in_features=hidden_size,
...
class PvtEncoder(nn.Cell):
    def __init__(self, config: PvtConfig):
        super().__init__()
        self.config = config

        # stochastic depth decay rule
        drop_path_decays = ops.linspace(
            0, config.drop_path_rate, sum(config.depths)).tolist()
        ...
        for i in range(config.num_encoder_blocks):
            embeddings.append(
                PvtPatchEmbeddings(
                    config=config,
                    image_size=config.image_size if i == 0 else self.config.image_size // (
                        2 ** (i + 1)),
        ...
        # Layer norms
        self.layer_norm = nn.LayerNorm(
            config.hidden_sizes[-1], epsilon=config.layer_norm_eps)
...
class PvtPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, cell: Union[nn.Dense, nn.Conv2d, nn.LayerNorm]) -> None:
        ...
        elif isinstance(cell, PvtPatchEmbeddings):
            cell.position_embeddings.data.initialize(
                Normal(self.config.initializer_range))
            if cell.cls_token is not None:
                cell.cls_token.data.initialize(
                    Normal(self.config.initializer_range))
