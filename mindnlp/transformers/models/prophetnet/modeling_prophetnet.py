...
def softmax(hidden_state, dim, onnx_trace=False):
    if onnx_trace:
        return ops.softmax(hidden_state.astype(ms.float32), axis=dim)
    else:
        return ops.softmax(hidden_state, axis=dim, dtype=ms.float32)
...
def ngram_attention_bias(sequence_length, ngram, dtype):
    ...
    """
    left_block = (
        ops.ones((ngram, sequence_length, sequence_length), dtype=dtype) *
        ms.tensor(np.finfo(ms.dtype_to_nptype(dtype)).min)
    )
    right_block = Tensor(left_block)
    # create bias
    for stream_idx in range(ngram):
        right_block[stream_idx].fill_diagonal(float(0), wrap=False)
...
def compute_relative_buckets(num_buckets, max_distance, relative_positions, is_bidirectional=False):
    ...
    if is_bidirectional:
        ...
        rel_positions_bucket = (
            rel_positions_bucket
            + ops.lt(inv_relative_positions,
                     ops.zeros_like(inv_relative_positions)).int() * num_buckets
        )
        inv_relative_positions = ops.abs(inv_relative_positions)
    else:
        inv_relative_positions = ops.maximum(
            inv_relative_positions, ops.zeros_like(inv_relative_positions))
    ...
    val_if_large = ops.minimum(val_if_large, ops.ones_like(
        val_if_large) * (num_buckets - 1)).int()
    rel_positions_bucket = rel_positions_bucket + \
        ops.where(is_small, inv_relative_positions.int(), val_if_large)
...
def compute_all_stream_relative_buckets(num_buckets, max_distance, position_ids):
    ...
    main_stream_relative_positions = position_ids.unsqueeze(
        1).tile((1, position_ids.shape[-1], 1))
    main_stream_relative_positions = main_stream_relative_positions - \
        position_ids.unsqueeze(-1)

    # predicting stream
    predicting_stream_relative_positions = ops.cat(
        (position_ids - 1, position_ids), axis=-1).unsqueeze(1)
    predicting_stream_relative_positions = predicting_stream_relative_positions.tile(
        (1, position_ids.shape[-1], 1))
...
class ProphetNetSeq2SeqLMOutput(ModelOutput):
    ...
    Args:
        ...
        logits (`ms.Tensor` of shape `(batch_size, decoder_sequence_length, config.vocab_size)`):
            Prediction scores of the main stream language modeling head (scores for each vocabulary token before
            SoftMax).
        logits_ngram (`ms.Tensor` of shape `(batch_size, ngram * decoder_sequence_length, config.vocab_size)`):
        ...
        past_key_values (`List[ms.Tensor]`, *optional*, returned when `use_cache=True` is passed or when `config.use_cache=True`):
            List of `ms.Tensor` of length `config.n_layers`, with each tensor of shape `(2, batch_size,
        ...
        decoder_ngram_hidden_states (`tuple(ms.Tensor)`, *optional*, returned when `output_hidden_states=True` is passed or when `config.output_hidden_states=True`):
            Tuple of `ms.Tensor` (one for the output of the embeddings + one for the output of each layer) of
        ...
        decoder_attentions (`tuple(ms.Tensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):
            Tuple of `ms.Tensor` (one for each layer) of shape `(batch_size, num_attn_heads,
            decoder_sequence_length, decoder_sequence_length)`.

            Attentions weights of the decoder, after the attention softmax, used to compute the weighted average in the
            self-attention heads.
        decoder_ngram_attentions (`tuple(ms.Tensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):
            Tuple of `ms.Tensor` (one for each layer) of shape `(batch_size, num_attn_heads,
            decoder_sequence_length, decoder_sequence_length)`.
        ...
        encoder_last_hidden_state (`ms.Tensor` of shape `(batch_size, encoder_sequence_length, hidden_size)`, *optional*):
    ...
    logits_ngram: Optional[ms.Tensor] = None
    past_key_values: Optional[Tuple[ms.Tensor]] = None
    decoder_hidden_states: Optional[Tuple[ms.Tensor]] = None
    decoder_ngram_hidden_states: Optional[Tuple[ms.Tensor]] = None
    decoder_attentions: Optional[Tuple[ms.Tensor]] = None
    decoder_ngram_attentions: Optional[Tuple[ms.Tensor]] = None
...
class ProphetNetSeq2SeqModelOutput(ModelOutput):
    ...
    Args:
        last_hidden_state (`ms.Tensor` of shape `(batch_size, decoder_sequence_length, hidden_size)`):
            Sequence of main stream hidden-states at the output of the last layer of the decoder of the model.

            If `past_key_values` is used only the last hidden-state of the sequences of shape `(batch_size, 1,
            hidden_size)` is output.
        last_hidden_state_ngram (`ms.Tensor` of shape `(batch_size,ngram * decoder_sequence_length, config.vocab_size)`, *optional*):
    ...
    last_hidden_state_ngram: Optional[ms.Tensor] = None
    past_key_values: Optional[Tuple[ms.Tensor]] = None
    decoder_hidden_states: Optional[Tuple[ms.Tensor]] = None
    decoder_ngram_hidden_states: Optional[Tuple[ms.Tensor]] = None
    decoder_attentions: Optional[Tuple[ms.Tensor]] = None
...
class ProphetNetDecoderModelOutput(ModelOutput):
    ...
    Args:
        last_hidden_state (`ms.Tensor` of shape `(batch_size, decoder_sequence_length, hidden_size)`):
            ...
            If `past_key_values` is used only the last hidden-state of the sequences of shape `(batch_size, 1,
            hidden_size)` is output.
        last_hidden_state_ngram (`ms.Tensor` of shape `(batch_size, ngram * decoder_sequence_length, config.vocab_size)`):
            Sequence of predict stream hidden-states at the output of the last layer of the decoder of the model.
        past_key_values (`List[ms.Tensor]`, *optional*, returned when `use_cache=True` is passed or when `config.use_cache=True`):
            List of `ms.Tensor` of length `config.n_layers`, with each tensor of shape `(2, batch_size,
            num_attn_heads, decoder_sequence_length, embed_size_per_head)`).
        ...
        hidden_states (`tuple(ms.Tensor)`, *optional*, returned when `output_hidden_states=True` is passed or when `config.output_hidden_states=True`):
            ...
            Hidden-states of main stream of the decoder at the output of each layer plus the initial embedding outputs.
        ngram_hidden_states (`tuple(ms.Tensor)`, *optional*, returned when `output_hidden_states=True` is passed or when `config.output_hidden_states=True`):
            Tuple of `ms.Tensor` (one for the output of the embeddings + one for the output of each layer) of
            shape `(batch_size, ngram * decoder_sequence_length, hidden_size)`.
        ...
        attentions (`tuple(ms.Tensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):
            ...
            Attentions weights of the decoder, after the attention softmax, used to compute the weighted average in the
            self-attention heads.
        ngram_attentions (`tuple(ms.Tensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):
            Tuple of `ms.Tensor` (one for each layer) of shape `(batch_size, num_attn_heads,
            decoder_sequence_length, decoder_sequence_length)`.
    ...
    hidden_states_ngram: Optional[Tuple[ms.Tensor]] = None
    attentions: Optional[Tuple[ms.Tensor]] = None
    ngram_attentions: Optional[Tuple[ms.Tensor]] = None
    cross_attentions: Optional[Tuple[ms.Tensor]] = None
...
class ProphetNetPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, cell):
        if isinstance(cell, nn.Dense):
            cell.weight.data.initialize(Normal(self.config.init_std))
            ...
            if cell.padding_idx is not None:
                cell.weight.data[cell.padding_idx].fill(0)
    ...
    def _shift_right(self, input_ids):
        ...
        shifted_input_ids[..., 1:] = Tensor(input_ids[..., :-1])
        ...
        assert ops.all(shifted_input_ids >= 0).item(
    ...
    def construct(self, inputs_shape, attention_mask=None, past_key_values=None, position_ids=None):
        ...
        if position_ids is None:
            if past_key_values is not None:
                ...
                position_ids = ops.ones((1, 1), dtype=ms.int64) * (
                    int(self.padding_idx + num_input_ids)
                )
            else:
                if attention_mask is None:
                    attention_mask = ops.ones(inputs_shape, dtype=ms.int64)

                # retrieve position_ids from input_ids / attention_mask
                position_ids = (
                    ops.cumsum(attention_mask, axis=1).astype(
                        attention_mask.dtype) * attention_mask
...
class ProphetNetAttention(nn.Cell):
    ...
    def construct(
        ...
        attn_weights = ops.einsum(
            "bsij,bsjk->bsik", query_states, key_states.swapaxes(2, 3))
        expected_shape = (batch_size, self.num_attn_heads, tgt_len, src_len)
        if attn_weights.shape != expected_shape:
            raise ValueError(
                f"Attention weights should have size {expected_shape}, but is {attn_weights.shape}")
        ...
        if attention_mask is not None and attention_mask.shape != expected_shape:
            raise ValueError(
                f"Attention mask should have size {expected_shape}, but is {attention_mask.shape}")
        ...
        if layer_head_mask is not None:
            ...
            attn_weights_reshaped = layer_head_mask.view(
                1, -1, 1, 1) * attn_weights_reshaped
        ...
        expected_shape = (batch_size, self.num_attn_heads,
                          tgt_len, self.head_dim)
        if attn_output.shape != expected_shape:
            raise ValueError(
                f"`attn_output` should have shape {expected_shape}, but is of shape {attn_output.shape}")

        attn_output = attn_output.swapaxes(1, 2).reshape(
            batch_size, tgt_len, hidden_size)
        attn_output = self.out_proj(attn_output)

        attn_output = ops.dropout(
...
class ProphetNetNgramSelfAttention(nn.Cell):
    def __init__(self, config: ProphetNetConfig):
        ...
        self.relative_pos_embeddings = nn.Dense(
    ...
    def construct(
        ...
        if past_key_value is not None:
            ...
            main_key_states = ops.cat(
                (prev_main_key_states, main_key_states), axis=2)
            prev_main_value_states = past_key_value[1]
            main_value_states = ops.cat(
                (prev_main_value_states, main_value_states), axis=2)
        ...
        main_attn_weights = ops.einsum(
            "bntc,bncs->bnts", main_query_states, main_key_states.swapaxes(2, 3))
        ...
        ).astype(main_attn_weights.dtype)
        ...
        predict_key_states = ops.stack(
            [ops.cat([main_key_states, key], 2) for key in predict_key_states_list], 1)
        ...
        predict_value_states = ops.cat(
            [ops.cat([main_value_states, v_p], 2).unsqueeze(2)
             for v_p in predict_value_states_list], 2
        )
        ...
        predict_attn_weights = ops.einsum(
        ...
        predict_attn_probs = softmax(
            ...
            dim=-1,
            onnx_trace=self.onnx_trace,
        ).astype(predict_attn_weights.dtype)
        ...
        predict_attn_output = ops.einsum(
            "bnhts,bnhsc->bnhtc", (predict_attn_probs,
                                   predict_value_states.swapaxes(1, 2))
        )
        ...
        attn_output = ops.cat([main_attn_output, predict_attn_output], 1).view(
    ...
    def get_main_relative_pos_embeddings(
        ...
        attn_weights = attn_weights.view(
            batch_size, num_attn_heads, tgt_len, src_len)
        ...
        rel_pos_embeddings = rel_pos_embeddings.reshape(
            attn_weights.shape[:3] + (-1,))

        main_relative_position_buckets = main_relative_position_buckets.tile(
        ...
        main_relative_pos_embeddings = ops.gather_elements(
...
class ProphetNetEncoder(ProphetNetPreTrainedModel):
    ...
    def __init__(self, config: ProphetNetConfig, word_embeddings: nn.Embedding = None):
        ...
        self.layers = nn.CellList([ProphetNetEncoderLayer(
    ...
    def construct(
        ...
        if attention_mask is not None:
            extended_attention_mask = (
                1.0 - attention_mask[:, None, None, :].tile(
...
class ProphetNetDecoder(ProphetNetPreTrainedModel):
    ...
    def __init__(self, config: ProphetNetConfig, word_embeddings: Optional[nn.Embedding] = None):
        ...
        self.layers = nn.CellList([ProphetNetDecoderLayer(
    ...
    def construct(
        ...
        if past_key_values is not None:
            ...
            ngram_hidden_states = [
                (ngram_embeddings[ngram - 1] +
                 predicting_stream_pos_embed).tile((batch_size, 1, 1))
        ...
        if encoder_attention_mask is not None:
            extended_encoder_attention_mask = (
                1.0 - encoder_attention_mask[:, None, None, :].tile(
    ...
    def compute_buffered_relative_buckets(self, position_ids):
        ...
        main_relative_buckets = main_relative_buckets[:, :sequence_length, :sequence_length].tile(
            (batch_size, 1, 1))
    ...
    def prepare_attention_mask(self, hidden_states, attention_mask):
        ...
        causal_mask = ops.full(
            ...
            ms.tensor(np.finfo(ms.dtype_to_nptype(hidden_states.dtype)).min),
        ...
        if attention_mask is not None:
            extended_attention_mask = (1.0 - attention_mask[:, None, None, :]) * ms.tensor(
    ...
    def prepare_predict_attention_mask(self, hidden_states, attention_mask):
        ...
        # add usual attention mask
        if attention_mask is not None:
            extended_attention_mask = (1.0 - attention_mask[:, None, None, None, :]) * ms.tensor(
                np.finfo(ms.dtype_to_nptype(self.dtype)).min)
...
class ProphetNetModel(ProphetNetPreTrainedModel):
    _tied_weights_keys = ["encoder.word_embeddings.weight",
...
class ProphetNetForConditionalGeneration(ProphetNetPreTrainedModel):
    _tied_weights_keys = ["encoder.word_embeddings.weight",
                          "decoder.word_embeddings.weight", "lm_head.weight"]
    ...
    def _tie_weights(self):
        if self.config.tie_word_embeddings:
            self._tie_or_clone_weights(
                self.prophetnet.word_embeddings, self.lm_head)
    ...
    def _compute_loss(self, logits, labels, ignore_index=-100):
        expend_targets = labels.new_zeros(
            self.config.ngram, labels.shape[0], labels.shape[1]).fill_(ignore_index)
...
class ProphetNetForCausalLM(ProphetNetPreTrainedModel):
    ...
    def _compute_loss(self, logits, labels, ignore_index=-100):
        expend_targets = labels.new_zeros(
            self.config.ngram, labels.shape[0], labels.shape[1]).fill_diagonal(ignore_index)
